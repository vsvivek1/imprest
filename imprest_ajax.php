...
echo $qry;
				...
				case "btn_show_aru_balance":

				imprestN::getAruBalanceAll();
...
case "btn_aquire_role_from_history":
	$qry = "select * from vw_office_setup where office_code='$_POST[office_code]' and branch_id='$_POST[branch_id]'   and is_live";
		$db = new DBAccess;
		...
		if ($row1['EOF']) {
			...
			?>
				...
				<div class="alert alert-success">
					...
					<span class="fa fa-angle-double-left"> Acquired successfully <?php echo  $_SESSION[full_name].' of  '.$row[branch] . $row[designation_id] . $row[office_name] ?></span>
...
break;
	...
	case 'btn_show_super_admin':
		...
		?>
								...
								";
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											...
											<input required="required" spellcheck="true" type=textarea id="txt_search_office" />
			...
			<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3">
				...
				<button type="button" id=btn_show_aru_balance class="btn btn-danger ">SHOW aru balance </button>
	...
	where imp_voucher_id=$imp_voucher_id returning *";
			...
			<!--
				...
				<table class="table table-stripped table-bordered">
					...
					<tr class="text-success lead"><td >Description</td><td colspan=3>
						...
						<textarea spellcheck="true" name=txt_description_imprest id=txt_description_imprest class="form-control">
						<?php echo $row[item_desc]; ?>
						</textarea></td></tr>
		...
		case "btn_edit_voucher":
			...
			if($status==1){
//if not submited get i month and i year from a_imprest_details 

$qry2="select * from a_imprest_details where imp_holder='$_SESSION[user_name]' and imp_holder_office='$_SESSION[office_code]' and status=1";
...
//if submited get i month and i year from a_imprest_voucher
$qry2="select * from a_imprest_details where imprest_ref_id='$imprest_num'";


			}elseif($_SESSION['aquired']==1 &&  $status==3){
				$qry2="select * from a_imprest_details where imprest_ref_id='$imprest_num'";
			...
			$db2 = new DBAccess;
			$row1x = $db2->SelectData($qry2);
			$rowx = $row1x[0];

			$i_month=$rowx['i_month'];
			$i_year=$rowx['i_year'];
			

			?>
			<input type="hidden" id="i_month" value="<?php echo $i_month ?>" >
			<input type="hidden" id="i_year" value="<?php echo $i_year ?>" >
			...
			//set min and max date for date of payment 
			...
			<table class="table table-stripped table-bordered">
				...
				<tr class="text-success lead">
					...
					<td colspan=3>
						...
						<textarea spellcheck="true" name=txt_description_imprest id=txt_description_imprest1 class="form-control">
						<?php echo $row[item_desc]; ?>
						</textarea></td>
				</tr>
...
inner join data_master dm on dm.dm_id=th.dm_id where trans_type=104 and visible=true and dm.loc_code=$_SESSION[location_code] 	and officer_privillage='3'
 group by item_acc_code, acc_head order by total desc";
			...
			// echo $_POST[option];
				...
				// echo "<br> $_SESSION[imp_fy]";  	/// imp from request
				...
				if($fyActual<>$_SESSION['imp_fy'] ){
					...
					// echo "inside-actual-".$fyActual." session". $_SESSION['imp_fy'] ;

					// if actual fy and fy from the request , ie from session are not  same 
							/// set date string as last date of financial year

							//set this $_POST[txtDate]

							$year1=explode('-',$_SESSION['imp_fy']);
							$year=$year1[1];
							$month=03;
							$day=31;
							...
							// echo $_POST[txtDate]; echo "<br>";
						
							$_POST[txtDate]=$CurrDate; // nailing curr date as march 31 of financial year ///

							// echo $_POST[txtDate]; echo "<br>";


				}else{
/// use actual txt date from post 

$_POST[txtDate]=$_POST[txtDate]; // no operation/dummy operation here for understading for coders other than vivek
...
if($_POST[option]==$_SESSION[option])
{
	unset($_SESSION["option"]);
				...
				$typeX = $empcode1X[2];
					...
					//////////////////////descriptuion new code overriding from client///////////////////////


					$_POST['txtdesci']=$_POST['txtdesci'];

					// $empname=imprestN

					$emp_name = imprestN::getEmpNameFromEmpCode($imp_holderX);

					$office_name = imprestN::get_office_name($original_officeX);

					
$qry="select i_month,i_year from a_imprest_details where imprest_ref_id='$imprest_id_refX'";
$db = new DBAccess;
		$row1 = $db->SelectData($qry);
		if(!isset($row1['EOF'])){
			$row=$row1[0];
			...
			$montDetails=" the month of $month/$year for" ;
		}else{
			...
			$montDetails="";
					...
					switch ($typeX){
						...
						case "P":
							$descriptionInVoucher="Being the Fresh Imprest Request of $emp_name of $office_name for the Finacial Year $_SESSION[imp_fy]";

						break;
						case "V":
							$descriptionInVoucher="Being the Recoupment of Imprest Vouchers  of $emp_name of $office_name for 
							
							$montDetails 
							the Finacial Year $_SESSION[imp_fy]";

						break;
						
						case "VC":

							$descriptionInVoucher="Being the Final Closing of Imprest Vouchers  of $emp_name of $office_name for the Finacial Year $_SESSION[imp_fy]";
						break;
					...
					if (isset($descriptionInVoucher)){
						$_POST['txtdesci']=$descriptionInVoucher;

					}else{

						// $_POST['txtdesci']=$_POST['txtdesci'];
						$_POST['txtdesci']="Being the Imprest Request of $emp_name of $office_name for $montDetails the Finacial Year $_SESSION[imp_fy]";
						
						
						//dummpy operation for others to c
					}
...
'$msg','$imprest_ref_id',false,'$fy'
	...
	$fy=imprestN::findFinancialYear($date);
						...
						if ($type == 'V') {
							$msg = "Dear $name_of_employee,\nYour imprest Recoupment of Amount $passed_amount /- has been Passed with Voucher Number $voucher_num.\nCheque will be issued soon by $branch_name of  $office_name";
						
							$msg="Dear $name_of_employee,Your imprest Recoupment of Amount $passed_amount/- has been Passed with Voucher Number $voucher_num. Cheque will be issued soon by $branch_name of $office_name";
						
						} elseif ($type == 'VC') {

							$msg = "Dear $name_of_employee,\nYour imprest of $fy is closed";

							$msg="Dear $name_of_employee,Your imprest of $fy is closed";
						} elseif ($type == 'P') {
							$msg = "Dear $name_of_employee,\nYour Fresh imprest Request of Amount $passed_amount /- has been Passed with Voucher Number $voucher_num.\nCheque will be issued soon by $branch_name of  $office_name";
						
							$msg="Dear $name_of_employee,Your Fresh imprest Request of Amount $passed_amount/- has been Passed with Voucher Number $voucher_num.Cheque will be issued soon by $branch_name of $office_name";
				...
				if(isset($transid) and $transid<>-1){
					include_once('../class/ErpBill.class.php');
					$objErpBill = new ErpBill;
					
					$objErpBill->createPVForImprest($transid);
				}
...
if($_SESSION[office_code]==$_SESSION[aru_code]){ 
	...
	<table id=tbl_adjust class="table table-stripped table-bordered">
		...
		<tr class=tr_template>
			...
			<td><textarea spellcheck="true" class="text_area_voucher_remark form-control"><?php echo $rw1[item_desc];?></textarea></td>
			<td>
...
(
 ...
 vw_office_setup v on aio.to_office= v.office_code inner join dl_empl s on s.unique_code::text=SPLIT_PART(aio.imprest_id_ref,'/', 1)
  where action_pending='t' and date_part('day',age(now(),imp_opn_time))>0 and SPLIT_PART(imprest_id_ref,'/', 3)<>'VC1' and v.is_head_of_office
   )
...
$p=0;
				foreach ($row1 as $row) {
					...
					$msg="Dear $employee,You have $pending items pending in your imprest inbox for further action at $branch of $office_name.";
...
$qry2="select entity_name as ename,phone,branch,office_name from a_personal_contacts apc inner 
join vw_office_setup vos on vos.user_name=apc.empcode::text  where previlege_id=22;";
///// all das /////////////////


				$db = new DBAccess;
				//echo $qry;
				$row1 = $db->SelectData($qry1);
...
$p=0;
				foreach ($row1 as $row) {
					...
					// msg to imprest holders with balance  final closing with amount 
...
$msg="Dear $employee,Important reminder! Please upload all vouchers and Close the Imprest before March 31st.Balance amount in your account is Rs$amount/- for $office_name";

// msg to imprest holders with balance  final closing with amount 

	$msg="Dear $employee,\n !! Important message regarding Imprest Software. All Permanent imprests have to be closed before March 31st. Imprest holders should upload their vouchers to the system well before march 31st for being accounted in the correct FY. If an imprest holder intends to final close, but have accidentally submitted in RECOUPMENT MODE, ARU staff can convert the same into FINAL CLOSING by pressing convert to the final closing button placed in the bottom & no need to return to the field. If imprests are submitted in multiple batches, ALL OF THEM have to be in the FINAL CLOSING mode. Inform the imprest holders to send imprest in final closing mode to avoid confusions and unwanted rework.Please Ask Imprest holders to upload their vouchers well before march 31st.\nRITU Kozhikode";
	
	$msg="Dear $employee,\n !! Important message regarding Imprest Software. Kindly do not return imprests to feild that have any issues with closing/opening  balance or duplicate entry. Contact RITU in the given Whatsapp number before proceeding.\n Kindly ensure that complete process of closing   .\nRITU Kozhikode";
			
	$msg="Dear $employee,\nPlease close your imprest well before March 31st for the FY 2019-20\n
	Steps for final closing\n
	1. Upload vouchers normally\n
	2. In the submit vouchers menu after verifying the cash book, press the button for final closing\n
	3. Enter the details of balance amount remitted if any in the respective columns\n
	4. Even If there is nothing to enter or no balance amount,keep the default values and press the Save button\n
	5. Submit to Higher office as usual.\n
	6. In any case, DO NOT upload the photo of the receipt of balance paid to ARU in the normal upload section, however, enter the details in the remittance details section.\n
	7. Make sure that your imprest reaches the ARU before March 31st\n";
...
inner join dl_empl s on s.unique_code=SPLIT_PART(aio.imprest_id_ref,'/', 1)::integer
 ...
 ";
			...
			foreach ($row1 as $row) {
				...
				$msg="Dear $employee,Your Imprest is with $with at $branch of $office_name from $from_date.";
				//imprestN::send_sms($phone,$msg);
				//imprestN::send_sms(+919847599946,$msg.$phone);
...
($imp_holder==$_SESSION[user_name])){
					...
					<table id=tbl_remitance_details_final_closing style=<?php echo $style; ?> class="table table-bordered table stripped ">
						<tr>
							...
							<td> <input type=text class='<?php echo $class; ?>' <?php echo $disabled; ?> name=txtremit id=txt_txtremit 
							value=<?php
							...
							<td> <input class='<?php echo $class; ?>' <?php echo $disabled; ?> 
							value='<?php 
							

							$row[voucher_num]=isset($row[voucher_num])?$row[voucher_num]:'NIL';
							echo $row[voucher_num];
							
							
							
							?>' type=text name=txtremitrpt id=txt_txtremitrpt></td>
						</tr>
						<tr>
...
$dt=date('d/m/Y');
$row[date_of_payment]=isset($row[date_of_payment])?$row[date_of_payment]:$dt ;
...
<?php $row[item_acc_code]=isset($row[item_acc_code])?$row[item_acc_code]:'saras';
...
";
		...
		case "tr_input_box":
			...
			//////////////////// setting session with imp fy////////////////////////

			$imp_fy=imprestN::findFinancialYearFromRef($_POST[imp_ref_id]);
			
			$_SESSION['imp_fy']=$imp_fy;

			// $f=imprestN::findFinancialYear();
			// echo $f;
...
$fy = imprestN::findFinancialYear($date);
			...
			
			{
				...
				if ($type == 'V') {
					...
					$btn_special_class = "btn-primary btn  btn-block2  toggle_v_to_vc";
				} elseif($type == 'VC') {
					$btn_text = "Edit  Final Closing details";
					$btn_special_class = "btn-warning btn  btn-block2  toggle_v_to_vc";
				}
			}
			 else {
...
<?php 
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 block-center" style="margin:10px">
						<button type="button"
						 ...
						 id='btn_convert_to_recoupment' class="btn-success btn  btn-block2">
...
";
	...
	case "check_any_voucher_uploaded":
		...
		$qry = "select count(*) from a_imprest_voucher 
		where imp_holder='$_SESSION[user_name]' and imp_holder_office='$_SESSION[office_code]'  
and  type is null and
voucher_status=1 and imp_fy='$fy'";
