...
<meta http-equiv='pragma' content='no-cache'> -->
		...
		<script src=\"../bootstrap/bootstrap-datepicker.js\"> </script> <link href=\"../bootstrap/bootstrap-datepicker.css\" rel=\"stylesheet\" type=\"text/css\"> <script src="../bootstrap/Chart.min.js"></script>
		<script src=\"../bootstrap/bootstrap-datepicker.js\"> </script> <link href=\"../bootstrap/bootstrap-datepicker.css\" rel=\"stylesheet\" type=\"text/css\"> <script src="../bootstrap/jquery.js"></script>
	...
	if ($_GET['frombtn'] <> '1') {
		...
		$navbar_fixed_top = 'navbar-fixed-top navbar-inverse';
	}
...
$(document).on("click", "#btn_show_aru_balance", function() {

// var imprest_id=$(this).val();

// var imprest_holder = $("#search_a_imprest").val();
var option = "btn_show_aru_balance";
...
$.ajax({
	...
	<script>
		$(document).on("click", "#btn_search_aimprest", function() {

			// var imprest_id=$(this).val();

			var imprest_holder = $("#search_a_imprest").val();
			var option = "btn_search_aimprest";
			// $(this).text("Make sleep");
			// $(this).removeClass("btn btn-success btn_make_live");
			// var ele=$(this);
			...
			$.ajax({
				...
				data: {
					option: option,
					imprest_holder: imprest_holder
				},

				success: function(html) {
					// ele.addClass("btn btn-warning btn_make_live");
					$('#div_a_imprest_out').html(html);
		...
		$(document).on("click", ".btn_del_a_imprest", function() {

			// var imprest_id=$(this).val();

			var imprest_id = $(this).val();
			var option = "btn_del_a_imprest";
			// $(this).text("Make sleep");
			// $(this).removeClass("btn btn-success btn_make_live");
			// var ele=$(this);
			...
			$.ajax({
				...
				success: function(html) {
					// ele.addClass("btn btn-warning btn_make_live");
					$('#div_a_imprest_out_response').html(html);
...
$.ajax({
	...
	<script>
		...
		$(document).on("click", "#btn_alert_pending_sms_to_feild", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "alert_pending_sms_to_feild"
				},
				beforeSend: show_ajax_loading_image(),
		...
		$(document).on("click", "#btn_show_pending_action", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_show_pending_action"
				},
				beforeSend: show_ajax_loading_image(),
	...
	<script>
		...
		$(document).on("click", "#btn_create_landing_rule", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_create_landing_rule"
				},
				beforeSend: show_ajax_loading_image(),
		...
		$(document).on("click", ".btn_save_imprest_landing", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_save_imprest_landing",
	...
	<script>
		...
		$(document).on("input", "#txt_paid_to,#txt_date_of_payement,#txt_amount_imprest,input[name=purpose]", function()
		...
		$(document).on("input", "#txt_voucher_num1,#txt_paid_to1,#txt_date_of_payement2,#txt_date_of_voucher1,#txt_amount_imprest1,#txt_purpose", function()
		...
		$(document).on("keyup", "#txt_date_of_payement, #txt_date_of_voucher,#txt_date_of_payement2,#txt_date_of_voucher1", function() {
		...
		$(document).on("keyup", "#txt_amount_imprest,#txt_amount_imprest1", function()
			...
			$.ajax({
				...
				success: function(html) {
					...
					var i_month = $("#i_month").val();
						...
						$("#txt_date_of_payement2").data("date-start-date", mindate);

						$("#txt_date_of_payement2").data("date-end-date", maxdate);

						$("#txt_date_of_voucher1").data("date-end-date", maxdate);
...
$(document).on("click", 
".btn_aquire_role_from_history", function() {
...
var option = "btn_aquire_role_from_history";
	...
	<script>
		...
		$(document).on("click", ".btn_remove_sup_doc", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_remove_sup_doc",
					"imp_file_id": imp_file_id
				},
				beforeSend: show_ajax_loading_image(),
		...
		function carosal_load() {
			...
			voucher_date=(voucher_date==undefined)?'2020-01-03':voucher_date;

			console.log("voucher date",voucher_date);
			var voucher_payement_date = $(".item.active").data("voucher-date-payment");

			voucher_payement_date=(voucher_payement_date==undefined)?'2020-01-03':voucher_payement_date;
			console.log("voucher payment date",voucher_payement_date);
	...
	*/
			...
			*/
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "view_voucher_del_button",
					imp_voucher_id: $(".item.active").data("voucher-id")
				},
				beforeSend: function() {
					var a = 0;
			...
			$.ajax({
				...
				data: {
					option: "get_supporting_documents",
					imp_voucher_id: $(".item.active").data("voucher-id")
				},
	...
	<script>
		...
		$(document).on("click", "#btn_show_div_history_classic", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_show_div_history_classic",
					imprest_id_ref: $(this).data("imprest_id_ref")
				},
				beforeSend: function() {
					var a = 0;
				},
		...
		$(document).on("click", "#btn_show_div_history_official", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_show_div_history_official",
					imprest_id_ref: $(this).data("imprest_id_ref")
				},
				beforeSend: function() {
					var a = 0;
				...
				$.ajax({
					...
					success: function(html) {
						...
						$.ajax({
							...
							type: 'POST',
							data: {
								option: "fill_audit_table_if_exists",
								imp_voucher_id: $(".item.active").data("voucher-id")
							},
							beforeSend: function() {
								var nop = 0;
...
*/
		...
		$(document).on("change", "#sel_imprest_type", function() {
			...
			{
				alert("Please select Imprest Type");
				return;
			}
	...
	<script>
		...
		$(document).on("click", ".tr_output_box1", function() {
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						option: "tr_output_box",
						imp_ref_id: imp_ref_id,
...

{
	...
	*/
		...
		$(document).on("click", ".btn_fwd_imprest", function() {
			...
			var imprest_ref_id = $("#txt_area_request_temp_imprest").data('imp-ref-id');
					...
					var msg2 = $("#m"+imp_op_id).html();

					msg1=msg2.replace('<br>','');
					// console.log(msg1);
					// alert(msg1);

					var amount_tot = $("[name=txtamount]").val();
					
					// var msg1=`${imprest_type} request of `
					msg = msg1 + '\n Amount :' + amount_tot + '\n Comments : ' + msg;


					//var to_office=$(this).attr("id");

					// alert(imp_op_id); return false;
	...
	*/
	...
	<script>
		$(document).one("click", "#btn_submit_perm_imp_req", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_submit_perm_imp_req",
					msg: msg,
					impType: impType,
					amt: amt
				},
	...
	<script>
		...
		$(document).on("keypress", "#txt_amount_imprest,#txt_cug,txt_otp_cug,txt_mobile,txt_otp_mobile,.item_amount1,#txt_amount_imprest1", function(event) {
	...
	<script>
		$(document).on("click", "#btn_prepare_abstract", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "prepare_abstract",
					imp_ref_id: imprest_ref_id
				},
				beforeSend: function() {
					$("#ajax_loading").modal('show')
				},
		...
		$(document).on("click", ".accord_link", function() {
			...
			$.ajax({
				...
				data: {
					...
					imp_status: imp_status,
					imp_op_id: imp_op_id,
					from_ofc_code: from_ofc_code,
					target: target,
					branch_id: branch_id
				},
				beforeSend: function() {
					show_ajax_loading_image_time();
	...
	<script>
		$(document).on("click", "#btn_convert_to_closing", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "show_option_to_close_imprest",
					imp_ref_id: imp_ref_id
				},
				beforeSend: function() {
		...
		$(document).on("click", "#btn_convert_to_recoupment", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_convert_to_recoupment",
					imprest_id_ref: imprest_id_ref
				},
				beforeSend: function() {
					show_ajax_loading_image();
	...
	<script>
...

		{
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_show_imprest_vouchers"
				},
				beforeSend: function() {
					var nop = 0;
				},
		...
		$(document).on("click", "#btn_del_voucher", function() {
			...
			if (confirm("Do You Really want to delete This Item")) {
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						option: 'btn_del_voucher',
						voucher_id: $(this).attr('name')
					},
	...
	<script>
		...
		$(document).on("keypress", "#txt_amount_imprest,.item_amount,#txt_amount_imprest1", function(event) {
...
<script>
$(document).on("keyup", "#txt_amount_imprest,.item_amount", function(event) {
...
let cr_balance=$("#cr_balance").text();
let dr_balance=$("#dr_balance").text();

let office_code=$(this).data("office-code");
...
if(office_code!=1330){
	...
	if (cr_balance>20000 || dr_balance>20000){

alert(`Verify Your debit and Credit amounts exceeds the limits your credit amount is ${cr_balance} and Debit amount is ${dr_balance}. \n Check whether there is any issue with the Finacial year.\n Try Log out and login  `);
return false;
			...
			// var count=$('[data-voucher-type=="paid"]').length;

			var c=$("*").filter(function(){
 return $(this).data('voucher-type')=='paid';

			});
			// alert(c.length);


var bal_hand=$("#td_cash_in_hand").html();

if(bal_hand<0){
	alert("You cannot submit vouchers exceeding cash in hand  with you ");
	return false;

}
			if(c.length==0){

				alert("To submit Imprest You must Enter atleast one payment Voucher");
				return false;
			}
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						option: "btn_submit_vouchers",
						amount: $(this).val(),
						vouchers_json: vouchers_json,
						note: note,
						imp_closing: imp_closing,
							...
							success: function(html) {
								...
								if (txtpayee == "-") {
									...
									if (imp_operation == "193" || imp_operation == "666") {
										...
										var error_div = "<div class='alert alert-danger'><span class='fa fa-stop-circle fa-2x'></span>&nbsp;<br>STOP!!<br>Payee is not correctly added.<br> Check the imprest holder details <br> verify the imprest holder is given correct employee code and correct office code <br> Issue with entity . Contact 9496 010 700  </div>";
    ...
    success: function(html){ 
		...
		*/
		...
		$(document).on("change", "#chkbox_close_imprest", function() {
			// var today = moment().format('DD/MMMYYYY');
...
// console.log(today);
			...
			if ($("#chkbox_close_imprest").is(':checked')) {
				...
				$("#txt_txtremit").val($("#td_cash_in_hand").html());
				$("#txt_txtremitrpt").val('NIL');
				$("#txt_date_of_remitance").val(today);
				// $("#sel_payament_mode").val('saras');

				var conf=confirm("Important Information. If you turn on this switch Your Vouchers Will not Receive Recoupment Amount for these Bills \n ഈ സ്വിച്ച് ഓൺ ചെയ്ത് കഴിഞ്ഞാൽ, നിങ്ങൾക്ക് ഈ ബില്ലുകൾക്കുള്ള ചെക്ക് , ARU വിൽ നിന്നും ലഭിക്കില്ല.");


				if(!conf){

					$("#chkbox_close_imprest").prop("checked",false);	
				}else{
					$("#chkbox_close_imprest").prop("checked",true);	
		...
		$(document).on("click", "#save_remitance_details ", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "save_remitance_details",
					source: $("#sel_payament_mode").val(),
					amount: $("#txt_txtremit").val(),
					receipt: $("#txt_txtremitrpt").val(),
					date: $("#txt_date_of_remitance").val()
				},
				beforeSend: show_ajax_loading_image(),
		...
		$(document).on("click", "#save_remitance_details_and_convert", function() {
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "save_remitance_details_and_convert",
					source: $("#sel_payament_mode").val(),
					amount: $("#txt_txtremit").val(),
					receipt: $("#txt_txtremitrpt").val(),
					date: $("#txt_date_of_remitance").val(),
	...
	<script>
...

		{
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					option: "btn_show_imprest_cash_book"
				},
				beforeSend: function() {
		...
		$(document).on("click", "#btn_show_imprest_cash_book,#btn_view_and_verify_cash_book", function() {
			btn_show_imprest_cash_book()
		});
		...
		$(document).on("click", "#btn_show_inbox,#btn_view_in_box", function() {
			show_in_box();
		});
	</script>
	...
	*/
			...
			$(document).on("click", "#edit_month_details", function() {
				...
				$.ajax({
					...
					type: 'POST',
					data: {
						option: "check_any_voucher_uploaded"
					},
					//beforeSend: show_ajax_loading_image(),
					...
					success: function(html) {
						...
						if (html == "ALLOW") {
							...
							alert("Already some vouchers are uploaded in current Month.\n\nIF you still wanted to change the current Month, Delete vouchers uploaded\n Tip. Do not attempt to  upload vouchers having different months of payment in one lot . Send them separatly ");
							stop_ajax_loading_image();
						}
			...
			$(document).on("click", "#btn_show_imprest_vouchers", function() {
				view_imprest_vouchers();
			...
			$(document).on("click", "#btn_show_imprest_vouchers2", function() {
				view_imprest_vouchers2();
							...
							<div class="collapse navbar-collapse" id="myNavbar" style="margin:auto">
								<ul id=ul_show_more class="nav navbar-nav" style="display: inline;" style="margin:auto">
									...
									if ($_SESSION[office_code] != $_SESSION[aru_code]) { ?>
										...
										$sum = imprestN::getSumOfIssuedImprestInFy(
											$_SESSION['user_name'],
											$_SESSION['office_code'],
											$freshIssuebeforeDate
										);
										...
										$qry = "select COALESCE (sum(amount),0) as sum from a_imprest_voucher where
...
";
										...
										// if ($_SESSION['user_name'] == 1040765) {

										// 	$ShowFreshRequestButton = true;
										// }

										//// check sum of closed amount in fy  as on today

										$qry = "select coalesce(sum(amount),0) as sum from a_imprest_voucher where
 imp_holder='$_SESSION[user_name]'
and imp_holder_office='$_SESSION[office_code]'
and imp_fy='$fy' and date_of_payment<CURRENT_DATE and split_part(imprest_num,'/',3)='VC' ";
										...
										//// get sum of fresh requested b4 today
										// and  combined sum of cah in hand today
										// get sum of cash in hand today = totsum


										// if tot sum equal to closed amnt  show frsh req btn
...
<!-- Imprest Voucher upload section is under maintanance . Other options will work -->
									...
									<?php
									if ((in_array($_SESSION['user_name'], array(1064767, 1058642, 1066892, 1046855, 1049072, 1042429, 1064380, 1040232, 1048592,1101486))) || $_SESSION[aquired] == 1)
								...
								<ul class='pull-right imprest_user_info'>
								<li style="display: inline;" class="list-inline-item" style="font-size:small">
										...
										if($_SESSION['aquired']==1)
										{
											$phone=imprestN::getpersonalContactFromEmployeeCode($empcode);
											
$cug=imprestN::getCugNumberFromOfficeCode($office_code);

											$contact="<br>Personal contact : $phone<br>CUG :$cug";
										...
										<div class="alert alert-primary">
											...
											<span style='font-size:11px'><?php echo $contact; ?></span>
...
<b class='bg-danger text-warning lead' style='color:red;'>
</b><br>
...
<?php
	...
	echo "<h1 style='color:blue;background:yellow;'>Imprest Software is under maintenance . Will be back soon . whatsapp 9847599946<h1>";
}
?>
...
<style>
	#myNavbar {
		...
		/* -webkit-box-shadow: 6px 9px 6px 0px rgba(46,7,176,0.75);
-moz-box-shadow: 6px 9px 6px 0px rgba(46,7,176,0.75);
box-shadow: 6px 9px 6px 0px rgba(46,7,176,0.75); */
	}
...
ul.imprest_user_info{

	margin: 0;
	padding:0;
}
ul.imprest_user_info li{
...
ul.imprest_user_info li div.alert{
...
.myBut,.fa-ksebl{
  -webkit-box-shadow: 6px 3px 9px 0px rgba(0,0,0,0.75);
-moz-box-shadow: 6px 3px 9px 0px rgba(0,0,0,0.75);
box-shadow: 6px 3px 9px 0px rgba(0,0,0,0.75);
}
