...
class imprestN
{
	...
	public static function getpersonalContactFromEmployeeCode($empcode){
	...
	public static function getCugNumberFromOfficeCode($office_code){

		$qry = "select cug from a_cug where office_id=$office_code";
	...
	public static function getAruBalanceAll($empcode = 0, $office_code = 0)
	{

		// getting ppl with balance in imprest ///

		$qry1 = "select * from (

			select sum(amount) as balance ,imp_holder,name as oname,ap.phone,ename,aiv.imp_holder_office
			...
			left join dl_empl d 
			 ...
			 where imp_fy='2020-2021'   group by imp_holder,oname,ap.phone,ename,aiv.imp_holder_office
...
$sl=1;
		...
		foreach ($rowy as $rw){
			...
			echo "<tr>";
									...
									$empcode=$rw['imp_holder'];
									$office_code=$rw['imp_holder_office'];

									// echo $empcode

									$row1 = self::imp_holder_details($empcode, $_SESSION[location_code]);
									$row1 = self::imp_holder_details_with_office2($empcode, $_SESSION[location_code], $office_code);
									$row = $row1[0];
									...
									echo "<td>$sl</td>"; $sl++;
									echo "<td>$office_code</td>";
									echo "<td>$empcode</td>";
									echo "<td>$dt </td>";
									echo "<td>----</td>";

									echo "<td>$rw[imp_holder] </td>";
									echo "<td>$netCode </td>";
								
									echo "<td>$rw[oname] </td>";
									echo "<td>$rw[balance] </td>";
									echo "<td>$Balance</td>";
...
/// imprest holders with balance ///
				...
				foreach ($row as $rw1) {
					...
					-->
						<td>
							...
							global $ttype;
							global $loccode;
							$qry = "select  acc_head,acc_code from trans_heads th inner join 
							data_master dm on dm.dm_id=th.dm_id where 

	trans_type=104 and visible=true and dm.loc_code=$loccode
	
	and acc_code not in [24210] and acc_code!='24210'
	order by acc_code";
	...
	imprestN::select2key($qry, "acc_code", "acc_head", "acc_code", "item_acc_head", "item_acc_head", $rw1[item_acc_code], "disabled=disabled");
		...
		-->
						...
						<td><textarea spellcheck="true" disabled=disabled class="text_area_voucher_remark form-control"><?php echo $rw1[item_desc]; ?></textarea></td>
...
-->
		...
		<caption>FY<?php echo $_SESSION['imp_fy']?></caption>
			<tr class='bg-success lead'>
				...
				<th>Balance After Passing this Item</th>
				<th>Will this Pass</th>
...
$imp_fy=$_SESSION['imp_fy'];
							$start1=explode('-',$imp_fy);
...
$budget_amount = $bal[amount];
								...
								$item_amount = $row['total'];
						
						
								$new_balance = $balance - $item_amount;

								// $after=$bal-$row['total'];
$passable=( $new_balance>0) ? 'Yes':'No';

$span=( $new_balance>0) ? '<span class="fa fa-check fa-2x  text-success"></span>':'<span class="fa fa-times fa-2x text-danger"></span>';
...
";
		...
		<div class="row">
			...
			<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 col-sm-offset-4">
				...
				<button type=button data-imp_op_id=<?php echo $post[imp_op_id]; ?>
				 data-imp_operation='999' data-imprest_ref_id='<?php echo $imprest_ref_id; ?>'
				 data-from_office='<?php echo $_SESSION[office_code]; ?>' data-from_branch='<?php echo $_SESSION[branch_id]; ?>' data-to_office='<?php echo $office_code; ?>'
				 data-to_branch='<?php
...
$btn_class_closing="btn-danger";


				} else {

					$pass_text = "Pass the bill For Payment";
					$btn_class_closing="btn-success";
				...
				<div id=show_submit_to_expac_after_abstract_prep>
					...
					-->
						...
						<tr>
							<td colspan=2>
								<textarea spellcheck="true" style="margin-bottom:50px" class=form-control name='txtdesci' cols=55 rows=10 class=form-control id=txt_area_voucher_note data-imp_ref_id='<?php echo $post[imp_ref_id]; ?>' data-to_office_id='<?php echo $_SESSION[office_code]; ?>' placeholder="Remarks ..."></textarea>
						...
						data-imp_operation=<?php echo $post[imp_operation]; ?>
			 			...
			 			Submit</button> -->
								...
								<button id=sub_to_expac
								 <?php $_SESSION[option] = "save_vouchers_in_saras"; ?>
								  data-imp_op_id=<?php echo $post[imp_op_id]; ?>
								   data-imp_operation=<?php echo $post[imp_operation]; ?> type=button class="btn  btn_fwd_voucher <?php echo $btn_class_closing;?>  ">
				...
				<div class="col-sm-10 col-sm-offset-1 well">
					...
					<table>
						...
						case 3:
							...
							<div class="row">
								<div class="co-sm-3"> <label for="inp_for_audit">For Audit</label></div>
								<div class="co-sm-3">
								<input class="auto_text_check_box_v form-control" id='inp_for_audit' type=checkbox value="&#10&#13For Audit">
								</div>
							</div>
					...
					<div align="center" style="margin:auto;text-align:center">
						<h5> Remarks ... </h5>
						<textarea  spellcheck="true" style="margin-bottom:50px" name='<?php echo $post[imp_ref_id]; ?>' data-imp_ref_id='<?php echo $post[imp_ref_id]; ?>' cols=55 rows=10 class=form-control id=txt_area_voucher_note placeholder="Remarks ..."></textarea>
...
if($imp_operation ==777){
	$sameOfficeOperation = true;
		...
		<div class="row">
			...
			<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 col-sm-offset-4">
				...
				<button  type=button data-imp_op_id=<?php echo $post[imp_op_id]; ?> 
				data-imp_operation='999' data-imprest_ref_id='<?php echo $imprest_ref_id; ?>' 
				data-from_office='<?php echo $_SESSION[office_code]; ?>' data-from_branch='<?php echo $_SESSION[branch_id]; ?>'
				data-to_office='<?php echo $office_code; ?>' data-to_branch='<?php
	...
	*/
	...
	public function  round_btn_help($id = "id", $class = "myBut", $toolTip = "click", 
	$fontAwesome = "fa fa-inr fa-5x", $iStyle = "color:white", $style = "", $content,$url)
	{
		echo  "<a id=$id
	  class=\"$class\" data-toggle=\"tooltip\" style=\"$style \" title=\"$toolTip\" data-placement=\"left\" href=$url> 
	  <i class=\"$fontAwesome\" style=\"$iStyle\"></i>
	$content </a>";
	}
   ...
   order by time,date_of_payment)y ";
	...
	(
		...
		if ($imp_ref_id == 0) {
			...
			$qry = "select coalesce(sum(amount),0) as sum from a_imprest_voucher
			 where imp_holder='$_SESSION[user_name]' and 
...
public static function showAlreadyClosedImprestInFy($imp_ref_id=0){

	$qry="
		select date_of_payment,amount,right(split_part(imprest_num,split_part(imprest_num,'/',1),3),15)::numeric as time2,
		 coalesce(type,'0')
		as type
		
		from a_imprest_voucher

		where 
		voucher_status=3 and 
		imprest_num!='$imp_ref_id' and 
		split_part(imprest_num,'/',3)='VC' and
		
		split_part(imprest_num,'/',1)=split_part('$imp_ref_id','/',1) and
		
		split_part(imprest_num,'/',2)=split_part('$imp_ref_id','/',2) and
		imp_fy='$fy' and coalesce(type,'p')<>'r' and 
		
		to_timestamp(right(split_part(imprest_num,split_part(imprest_num,'/',1),3),15)::float)
		
		< to_timestamp(right(split_part('$imp_ref_id',
		split_part('$imp_ref_id','/',1),3),15)::float)	)	";
		...
		if($_SESSION[aquired]==1)
{
	...
	// echo $qry;   .5 ms 
		...
		$fy=self::findFinancialYear();
		$tableHead = "Imprest CASH ACCOUNT (original Entries by Imprest holder) -Financial year $fy";
		$trclass = "voucher";
		$db = new DBAccess;
		$row1 = $db->SelectData($qry);
		//echo $qry;
...
$rowtype='paid';
					...
					$rowtype='received';
				}if($row[type]=='remitance'){
					$rowtype='received';

				}

				echo "<tr class=$trclass data-voucher-type=$rowtype id=$row[imp_voucher_id]>";
	...
	AND voucher_status=1 ";
				...
				if ($row[item_desc] == 'KSEB') {
					...
					if($_SESSION['aquired']==1){
						...
						<button id=btn_show-modal_edit_table><i class='fa fa-pencil' style='color:red'>Edit table</i>
				...
				if ($row[item_desc] <> 'KSEB') {
					...
					if($_SESSION['aquired']==1){
						...
						<button id=btn_show-modal_edit_table>
						<i class='fa fa-pencil' style='color:red'>Edit table</i>
	...
	//  echo "<td></td>";
			...
			echo "<tr><td class=text-primary colspan=3>Total</td> 
			<td class='text-success' id=dr_balance>$dr_balance</td>
			<td class='text-danger' id=cr_balance>$cr_total</td></tr>";
	...
	public static function findFinancialYear($date=0){

// echo "session value $_SESSION[imp_fy]";


		if(isset($_SESSION['imp_fy'])){
			
			return $_SESSION['imp_fy'];
...
/// finding set imp_fy by ae 

	$qry="select i_month,i_year from a_imprest_details
	 where imp_holder=$_SESSION[user_name] and imp_holder_office=$_SESSION[office_code] 
...
if(!isset($row1['EOF']))
{
	...
	$month=$row['i_month'];
...
else
		{
			...
			$fy=self::findFinancialYearActual($date);
	...
	public static function findFinancialYearActual($date)	
	{
	...
	public static function findFinancialYearFromRef($imprest_id_ref=0)

	{
		...
		{
					$date = date("Y-m-d");
					self:findFinancialYear($date);

		}
		else
			{

			$ar1=explode("/",$imprest_id_ref);
			$fy=$ar1[3];
...
// print_r($_SESSION);
					...
					-->
						...
						<td><textarea spellcheck="true" class="item_desc1 form-control"><?php echo $rw1[item_desc]; ?></textarea></td>
...
where o.code='$office_code'  and os.branch_id::int=$branch and os.is_live  ";
	...
	('$imp_operation','$_SESSION[office_code]','$to_office','$_SESSION[branch_id]','$to_branch','$_SESSION[user_name]','$msg','$imprest_ref_id','$fy'
		...
		<li><i class='fa fa-check fa-3x'>   <button class='btn btn-primary' id='btn_view_in_box'><i class='fa-mail fa-3x'></i>
		  Click Here to View Inbox</button></i></li>
			...
			$msg="Dear $name_of_employee,your imprest has been forwarded to $branch_name of $office_name. Your imprest is with $receiver";
			...
			if (!$inReceiversInBox) {
				...
				$msg="Dear $name_of_employee,your Imprest has been passed Cheque will be issued soon by $branch_name of $office_name. If you have not received cheque in 2 days,contact ARU immediately";
			}
			...
			if (!isset($row['EOF'])) {
				...
				$msg="Dear $to_employee, Imprest of $name_of_employee has been forwarded to you.Kindly check Your imprest software and do the needfull";
	...
	public static function submit_vouchers($post)
	{
		...
		/////////////////////////////scruipt blocjk for imp fy checking
		$date = date("Y-m-d");
		$fyActual=self::findFinancialYearActual($date);  //fy actual
		
		

		// if($fyActual<>$_SESSION['imp_fy'] ){

		// 	$post[imp_closing]=1;


		// }else{
		// 	$post[imp_closing]=$post[imp_closing]; //dummy operation
...
";
		...
		<li><i class='fa fa-check fa-3x'>   <button class='btn btn-primary' id='btn_view_in_box'><i class='fa-mail fa-3x'></i>
		  Click Here to View Inbox</button></i></li>
			...
			// $message="$_SESSION[user_name] submitted by $_SESSION[full_name] type $v $fy and imp $impReqNum";
			
			// self::send_sms(+919847599946, $message);
		}
	}
	...
	public static function show_initial_set_up_for_month()
	{
		...
		<div class="row">
			...
			<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 col-sm-offset-3">
				...
				<p3>
				Enter The Month in which you have made Payments.
				 Illustration (* You purchased several items in february,however at the time of purchase you dont have any balance imprest cash with you.
				  After that You  received the cheque in March. In this case, the  Month you have to select  is March )
				</p3>
				
				
								<div class='alert alert-danger'>ശ്രദ്ധിക്കുക... വർഷവും മാസവും തിരഞ്ഞെടുക്കുന്നതിനു മുമ്പ് അവ കൃത്യമാണ് എന്ന് ഉറപ്പുവരുത്തുക.. ഇത് വർഷം 
...
";
	...
	('$imp_operation','$_SESSION[office_code]','$to_office','$_SESSION[branch_id]','$to_branch','$_SESSION[user_name]','$msg','$imprest_ref_id','$fy'
		...
		$msg="Dear $name_of_employee, your Imprest Has been Returned to You by $office_name. Please check your Imprest Inbox and do the Needful";
		self::execute_sms($office_code_of_imperst_holder, $desig, $msg);
		$empcode = $office_code_of_imperst_holder1[0];
		self::execute_sms_personal($empcode, $desig, $msg);
...
*/
		...
		///////////////////////////////new methodd financial year from ref //////////////////////////////

		$fy=self::findFinancialYearFromRef($imprest_ref_id);
	...
	your imprest has been passed . Cheque will be issued soon by  $branch_name of  $office_name.";
				...
				if ($imp_operation != 200) {
					...
					$msg="Dear $to_employee,Imprest Vouchers of $name_of_employee has been forwarded to you. Kindly check your imprest software for the role of $to_branch_name_sms,$to_ofc_name_sms and do the needfull.";
					self::execute_sms_personal($empcode, $desig, $msg);
...
<?php
				...
				$date = date("Y-m-d");
				$fyActual=imprestN::findFinancialYearActual($date);  //fy actual
							

				$fySYS=imprestN::findFinancialYear($date);
				// echo "<BR>FY IN SYSTEM : -". $fySYS ;
				// echo "<BR>Actual : - $fyActual <BR>";
...
<ol>
	<li>Your Imprest FY seems to be set wrong . To solve this Do the following</li>
	<li>In submit Vouchers section : -Check if there is any pending Vouchers uploaded and Not submitted in Submit Voucher section
	 UPLOADED BY YOU other than Remitance.If there is any thing , delete it or submit it </li>
	<li>In upload voucher section check whether anything is there, in month and year Settings.If month and year is not anything after april
	<?php echo date('Y') ;?>  Change it to Month of current Financial Year.
	 in Edit Month Details </li>
	 <li>After these two steps if you still see this message Contact  RITU kozhikode in whatsapp or SMS  ONLY with Your Employeee Code </li>
</ol>
...
?>
					...
					<h5 class="text-warning"><?php 
					$fy=self::findFinancialYear();
					echo "Financial Year $fy";
...
";
	...
	public static function imp_holder_details_with_office2($empcode, $loccode, $office_code)
...
";
	...
	public static function getHistory($imp_ref_id)
	{
		...
		<div class="row" style="/*border:2px solid black;*/ padding-bottom:10px;">
			...
			<div class='col-sm-4 '>
				<button id=btn_show_div_history_classic data-imprest_id_ref="<?php echo $imp_ref_id; ?>" class='btn btn-primary'>
				<span class="fa fa-lg far fa-comment"></span>
					&nbsp;
...
?>
<button 
class="btn btn-link text-danger btn_aquire_role_from_history " 

data-office_code="<?php echo $row[from_office] ?>"
data-branch_id="<?php echo $row[from_branch] ?>"

>

<span class="fa fa-user-secret text-danger"> 
 ...
 [cmbsec] => 2303 [locid] => 323 [impchequeid] => [sbmt] => SAVE ) 
		...
		-->
						...
						<td colspan=2>
							...
							<textarea spellcheck="true" style="margin-bottom:50px" class=form-control name='txtdesci' cols=55 rows=10 class=form-control id=txt_area_request_temp_imprest data-imp-ref-id='<?php echo $imp_ref_id ?>' data-to_office='<?php echo $_SESSION[office_code] ?>' data-imp_op_id='<?php echo $imp_op_id; ?>' placeholder="Remarks ..."></textarea>
				...
				<div class=row>
					<div class="col-sm-10 col-sm-offset-1 well">
						...
						<div align="center" style="margin:auto;text-align:center">
							...
							<h5> Remarks ... </h5>
							<textarea spellcheck="true" style="margin-bottom:50px" name='<?php echo $x[imprest_id_ref]; ?>' cols=55 rows=10 class=form-control id=txt_area_request_temp_imprest data-imp_ref_id='<?php echo $post[imp_ref_id] ?>' data-imp_op_id='<?php echo $imp_op_id; ?>' placeholder="Remarks ..."></textarea>
...
$date = date("Y-m-d");
				$fyActual=imprestN::findFinancialYearActual($date);

				$yrs=explode('-',$fyActual);
...
if($_SESSION['aquired']==1){
	...
	$qry = "select imprest_op_id as \"operation\",
		...
		where from_office='$_SESSION[office_code]' and 
		from_branch='$_SESSION[branch_id]'   and (imp_fy='$fyActual' )  order by imprest_op_id desc";
		...
		*/
			...
			$tableHead = "Details of Outgoing actions of financial year $fyActual";
...
<div class="alert alert-danger">
<strong>Important Instruction</strong><br> Read CareFully
<ol>
	<li>Your Imprest FY seems to be set wrong . To solve this Do the following</li>
	<li>In submit Vouchers section : -Check if there is any pending Vouchers uploaded and Not submitted in Submit Voucher section
	 UPLOADED BY YOU other than Remitance.If there is any thing , delete it or submit it </li>
						...
						<div class="panel panel-default message" style="margin-top:2%">
							<div class="panel-heading" role="tab" id="<? echo "heading-$sl"; ?>">
								...
								<h4 class="panel-title"
								 id="<? echo $row[imp_status]; ?>" value="<? echo $row[imprest_op_id]; ?>" name="<? echo $row[imprest_op_id]; ?>">


									<a class="accord_link" 
									name='<? echo "$row[imprest_id_ref]"; ?>'
									 data-toggle="collapse" data-parent="#accordion"
									  href="<? echo "#collapse-$sl"; ?>" 
									  data-imprest-status="<? echo $row[imp_status]; ?>"
									   data-imprest-from_office="<? echo $row[from_office_code]; ?>" data-imprest_id_ref="<? echo $row[imprest_id_ref]; ?>" data-from_branch="<? echo $row[from_branch]; ?>" data-imp_op_id="<? echo $row[imprest_op_id]; ?>" data-imp_operation="<? echo $row[imp_operation]; ?>" aria-expanded="false" aria-controls="<? echo "collapse-$sl"; ?>" role="button">
...
<?php echo "$dispaly_voucher_type"; ?> by <?php echo "$emp_name"; ?> of <br><?php echo "$imprest_of_office_name"; ?></h3>
					...
					<div class=row>
						<div class="col-sm-10 col-sm-offset-1 well">
							...
							<div align="center" style="margin:auto;text-align:center">
								...
								<textarea spellcheck="true" cols=55 rows=10 class=form-control id=txt_area_request_perm_imprest placeholder="Remarks ..."></textarea>
							...
							<div class=row style="margin-top:5px">
								<div class="col-sm-10 col-sm-offset-2">
									<button id=btn_submit_perm_imp_req class="btn btn-primary " disabled="disabled">
				...
				$row = $db->SelectData($qry); { ?>
					...
					<div class=row>
						<div class="col-sm-10 col-sm-offset-1 well">
							...
							<div align="center" style="margin:auto;text-align:center">
								<h5> Remarks ... </h5>
								<textarea spellcheck="true" cols=55 rows=10 class=form-control id=txt_area_request_temp_imprest placeholder="Remarks ..."></textarea>
					...
					<table class='table table-stripped table-hovered table-bordered'>
						<tr class=bg-default>
							<td  clss=text-center>
								...
								<span class="bg-danger text-danger">If you do not need Recoupment or Payment , Turn on this button<span>
							
<p> If you have more than one month of imprest pending  for Final closing,
	 make sure that you ENABLE final closing button for each  of the imprest lots<br>
</p>
							</td>
							<td>
							...
							<td>
							<span id=span_final_closing_btn>Final Closing Button</span>
							<label class="switch">Close
							...
							<td colspan=2>
								...
								<table id=tbl_remitance_details style=<?php echo $style; ?> class="table table-bordered table stripped ">
									...
									<tr>
										...
										<?php if ($row[item_acc_code] == "") $row[item_acc_code]="saras"; ?> 
									...
									<tr>
										...
										If there is nothing to remit , save the default values
										</td>
									</tr>
				...
				#span_final_closing_btn,#btn_aquire_role_from_history{
  color:red;
		 ...
		 order by imprest_op_id desc limit 1";
				...
				<div class=row>
					<div class="col-sm-10 col-sm-offset-1 well">
						...
						<div align="center" style="margin:auto;text-align:center">
							<h5> Remarks ... </h5>
							<textarea spellcheck="true" style="margin-bottom:50px" name='<?php echo $post[imprest_id_ref]; ?>' cols=55 rows=10 class=form-control id=txt_area_voucher_note placeholder="Remarks ..." data-imp_ref_id='<?php echo $post[imp_ref_id]; ?>'></textarea>
					...
					<div class=row>
						<div class="col-sm-10 col-sm-offset-1 well">
							...
							<div align="center" style="margin:auto;text-align:center">
								...
								<h5> Remarks ... </h5>
								<textarea spellcheck="true" cols=55 rows=10 class=form-control id=txt_area_note_for_voucher placeholder="Remarks ..."></textarea>
							...
							<div class=row style="margin-top:5px">
								<div class="col-sm-10 col-sm-offset-2 block-center">
									...
									if ($_SESSION[office_code] == $_SESSION[aru_code]) {
										...
										data-office-code="<?php echo $_SESSION[office_code] ?>"
...
<div class="alert alert-danger">Higher Office Not Set . Contact Your ARU for setting your Higher Office Properly</div>
<?php
											 }
							...
							<div class="row">
								...
								<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
									<label for=""><span class='fa fa-bell text-danger'>&nbsp;Auto Description</span>&nbsp;<input checked='checked' type=checkbox id=chk_auto_description>
										&nbsp;Description</label>
									<textarea spellcheck="true" name=txt_description_imprest id=txt_description_imprest class="form-control"></textarea>
								</div>
...
";
					...
					<div class="col-sm-4 " id=div_msg>
					Dear <?php echo $_SESSION['full_name']; ?>,<br>

Please close your imprest well before March 31st for the FY 2020-21<br>
Steps for final closing<br>
1. Upload vouchers normally<br>
2. In the submit vouchers menu after verifying the cash book, press the button for final closing <br>
3. Enter the details of balance amount remitted if any in the respective columns. <span class=text-success>Mode of Payment Division cash=saras</span><br>.
4. Even If there is nothing to enter or no balance amount, press the save Button<br>
5. Submit to Higher office as usual.<br>
6. In any case do not upload the photo of the receipt of balance paid to ARU in the normal upload section, however, enter the details in the remittance details section.<br>
7. Make sure that your imprest reaches the ARU before March 31st<br>
8. If you have more than one month of imprest for Final closing, make sure that you ENABLE final closing button for both of the imprest <br>
					</div>
						...
						<form id=form1>
							<table border=1 class="table table-stripped table-bordered" style="border:1px solid blue;">
								...
								<tr class="text-success lead">
									...
									<td>Paid To</td>
									<td>
									<input list="venders" type=text name=txt_paid_to id=txt_paid_to class="form-control">
									
<?php

$qry="
select distinct paid_to from (

select trim(upper(paid_to)) as paid_to from a_imprest_voucher where imp_holder_office='$_SESSION[office_code]'



) w where paid_to not in ('$_SESSION[user_name]','0','O') and CHAR_LENGTH(paid_to)>3
 order by paid_to
";
$db = new DBAccess;
				$row = $db->SelectData($qry);
...
?>
<datalist id="venders" open="open">
  <?php

  foreach($row as $r1){

	echo "<option value='$r1[paid_to]'>";
  }
					...
					trans_type=104 and visible=true and dm.loc_code=$loccode order by acc_head";
								...
								<tr class="text-success lead">
									...
									<td colspan=3> 
									 ...
									 <input list="purpose" type=text name=purpose required=required class='form-control'>
...
$qry="
select distinct purpose from (

select trim(upper(purpose)) as purpose from a_imprest_voucher where imp_holder_office='$_SESSION[office_code]'



) w where purpose not in ('KSEB','0','O') and CHAR_LENGTH(purpose)>5 and purpose not like '%$_SESSION[user_name]%'
 order by purpose
";
$db = new DBAccess;
				$row2 = $db->SelectData($qry);
...
?>
<datalist id="purpose" open="open">
  <?php

  foreach($row2 as $r12){

	echo "<option value='$r12[purpose]'>";
  }
...
?>
								...
								<tr class="text-success lead">
									...
									<td colspan=3><textarea spellcheck="true" name=txt_description_imprest id=txt_description_imprest class="form-control"></textarea></td>
								</tr>
